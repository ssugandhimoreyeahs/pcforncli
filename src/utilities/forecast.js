...
export function getUpdatedForecast1(outOfCastDate, coh, expense, sales) {
  ...
  let remainingDays = new Date(
    new Date(outOfCastDate).getFullYear(),
    new Date(outOfCastDate).getMonth(),
    new Date(outOfCastDate).getDate() + finalDays
  );
  let updateDateOne = Moment(remainingDays).format("YYYY-MM-DD");
  console.log(
    "Moment working here --- ",
    Moment("2020-03-01").format("DD-MM-YYYY")
  );
  if (avgPerDay < 0) {
...
export function getUpdatedForecast3(
  outOfCastDate,
  coh,
  expense,
  sales,
  ...
  if (mainCoh < coh) {
    remainingDays = new Date(
      ...
      new Date(mainDate).getMonth(),
      new Date(mainDate).getDate() + finalDays
    );
  } else {
    remainingDays = new Date(
      new Date(mainDate).getFullYear(),
...
export function getUpdatedForecast(
  outOfCastDate,
  coh,
  expense,
  sales,
