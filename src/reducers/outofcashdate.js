...
export const outOfCashDateReducer = (state = initialState,action) => {
    const { type } = action;
    switch(type){
        case FETCH_OUT_OF_CASH_DATE_REQUEST: return {
            error: false,
            loading: true,
            fetched: false,
            outOfCashDateResponse: {}
        }
        case FETCH_OUT_OF_CASH_DATE_SUCCESS : return {
            error: false,
            loading: false,
            fetched: true,
            outOfCashDateResponse: action.outOfCashDateResponse
        }
        case FETCH_OUT_OF_CASH_DATE_ERROR : return {
            error: true,
            loading: false,
            fetched: false,
            outOfCashDateResponse: {}
...
export const outOfCashDateAsyncCreator = () => (dispatch) => {
    ...
    getCashOutOfDatePromise().then(response => {
