...
export const mainExpenseReducer = (state = initialState, action) => {
  switch (action.type) {
    case MAIN_EXPENSE_CATEGORY_REQUEST:
      return {
        error: false,
        errorMsg: "",
        loader: true,
    ...
    case MAIN_EXPENSE_CATEGORY_SUCCESS:
      return {
        error: false,
        errorMsg: "",
        loader: false,
    ...
    case MAIN_EXPENSE_CATEGORY_ERROR:
      return {
        error: true,
...
export const fetchMainExpenseAsyncCreator = (expenseType = 0) => {
  return (dispatch) => {
    ...
    getExpenseByCategoryScreenPromise(expenseType)
      .then((response) => {
        if (response.result == true) {
          dispatch(
            mainExpenseCategorySuccess({
