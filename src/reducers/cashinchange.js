...
export const cicReducer = (state = initialCICState, action) => {
  switch (action.type) {
    ...
    case CIC_FETCH_MULTIPLE_REQUEST:
      return {
        errorMsg: "",
        error: false,
        masterLoader: false,
...
export const cicAsynCreator = (cicCurrentRange = 0) => {
  return (dispatch) => {
    ...
    getChangeInCash(cicCurrentRange)
      .then((response) => {
        if (response.result == true) {
          ...
          dispatch(
            cicFetchSuccess({ cicData: cashInChangeResponse, cicCurrentRange })
          );
        } else {
          cicCurrentRange = cicCurrentRange == 0 ? 1 : cicCurrentRange;
          dispatch(
            cicFetchError({ errorMsg: "Error Try Again!", cicCurrentRange })
          );
        }
      })
      .catch((error) => {
