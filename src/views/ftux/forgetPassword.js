...
class Forgetpassword extends Component {
  ...
  handlePress = async () => {
    ...
    const forgetPasswordApiCall = await forgetPassword(
    ...
    if (forgetPasswordApiCall.result == true) {
      this.setState(
        (prevState) => {
          return { isSpinner: false, isButtonEnabled: false, email: "" };
        },
        () => {
          setTimeout(() => {
            Alert.alert(
      ...
      if (
        forgetPasswordApiCall.error.response != undefined &&
        forgetPasswordApiCall.error.response.status == 400
      ) {
        this.setState({
          isSpinner: false,
          invalidBusinessEmail: true,
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        />
        {this.state.invalidBusinessEmail && (
