...
export default class CashOnHandChart extends PureComponent {
  ...
  render() {
        ...
        for (let i = 0; i < this.props.cashOnHandGraphData.length; i++) {
          ...
          this.props.cashOnHandGraphData.map((singleGraph, index) => {
            ...
            if (i == index) {
              ...
              if (
                singleGraph.Amount == null ||
                singleGraph.Amount == undefined
              ) {
    ...
    var gradients = [];
    let {
      cashOnHandGraphData: forGradientData,
      cohFuture,
      cohPast,
    } = this.props;
        ...
        if (i == forGradientData.length - 1) {
          if (getAmmount > 0) {
            gradients.push(
              <Stop
                key={i}
                offset={`${precentForCurrentMonth * i}%`}
                stopColor={CUSTOM_GRADIENT["1"][i]}
              />
            );
          } else {
            gradients.push(
        ...
        if (i == forGradientData.length - 1) {
          if (getAmmount > 0) {
            gradients.push(
              <Stop
                key={i}
                offset={`${percentForTeweleMonth * i}%`}
                stopColor={CUSTOM_GRADIENT["12"][i]}
              />
            );
          } else {
            gradients.push(
              <Stop
                key={i}
                offset={`${percentForTeweleMonth * i}%`}
                stopColor={CUSTOM_GRADIENT["12"][i + 1]}
              />
            );
          }
        } else {
    ...
    if (applyGraph.length > 0) {
      ...
      let increment25Percent = parseInt(
        maximumValue + (maximumValue * 25) / 100
      );
    ...
    applyGraph = applyGraph.map((singleArray) =>
      singleArray.y >= 0 ? singleArray : { ...singleArray, y: 0 }
    );
    return (
      <Fragment>
        {this.props.cashOnHandGraphData.length > 0 ? (
          this.props.cohFuture == 0 && this.props.cohPast == 0 ? (
            ...
            >
              <Defs>
                <LinearGradient id="gradient">{gradients}</LinearGradient>
              </Defs>
              <VictoryAxis
                style={{
                  axis: { stroke: "none" },
          ...
          <VictoryChart height={260} width={gw} theme={VictoryTheme.material}>
            {/* <Defs>
             <LinearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
               {gradients}
             </LinearGradient>
           </Defs> */}
            <VictoryAxis dependentAxis={true} tickValues={[0]} />
