  ...
  render() {
    ...
    let allPositiveMax =
      allPositiveValues.length > 0
        ? Math.max(...allPositiveValues.map((singleData) => singleData.y))
        : 0;
    let allNegativeMax =
      allNegativeValues.length > 0
        ? Math.abs(
            Math.min(...allNegativeValues.map((singleData) => singleData.y))
          )
        : 0;
    console.log(
      "-----------all Postiive and negative value finalization ---- "
    );
    console.log("allPositiveMax - ", allPositiveMax);
    console.log("allNegativeMax - ", allNegativeMax);
    console.log("--------------------------ends here-------------------");
    let aheadDataMaxValue =
      allPositiveMax >= allNegativeMax ? allPositiveMax : allNegativeMax;

    let aheadDataMaxValue25Percent = parseInt(
      (aheadDataMaxValue + (aheadDataMaxValue * 25) / 100) / 2
    );
          ...
          domainPadding={{
            x:
              cashInChangeData.cicCurrentRange == 1
                ? 10
                : cashInChangeData.cicCurrentRange == 3
                ? 20
                : 15,
          }}
          animate={{
