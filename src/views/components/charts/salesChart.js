  ...
  render() {
    ...
    if (this.props.salesData.length > 0) {
      ...
      this.props.salesData.map((singleSalesData) => {
        let obj = {};
        obj.x =
          this.props.salesCurrentRange == 1
            ? new Date(singleSalesData.date).getDate()
            : singleSalesData.month;
        //console.log(obj.x);
        obj.y = parseFloat(singleSalesData.amount);
        applySalesGraph.push(obj);
      });
    }
    let YAXISValue = applySalesGraph.map(
      (singleSalesValue) => singleSalesValue.y
    );
      ...
      <Fragment>
        {this.props.salesData.length > 0 ? (
          ...
          >
            ...
            <VictoryBar
              barRatio={barRatio}
              data={applySalesGraph}
              style={{ data: { fill: "#1188DF" } }}
            />
          </VictoryChart>
        ) : null}
      </Fragment>
