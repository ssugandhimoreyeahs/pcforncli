...
class ChangeInCash extends Component {
  ...
  showAlert() {
    Alert.alert(
      TERMINOLOGY.CHANGEINCASH.title,
      TERMINOLOGY.CHANGEINCASH.message,
  ...
  changeInCashMasterLoader = React.memo(() => {
    return (
      <View style={styles.cicMasterLoader}>
  ...
  renderCiCBody = React.memo(
    ({ height, isCICGraphEmpty, total, historicalFinances }) => {
      return (
        <View style={{ height }}>
          ...
          {isCICGraphEmpty == true ? (
            <this.renderCicGraphEmpty />
          ) : (
            <View
              style={{ marginTop: "-5.5%", marginLeft: "-10%" }}
              accessible={true}
              pointerEvents="none"
            >
        ...
        <View style={styles.buttonview}>
          ...
          >
            <Dropdown
              ...
              data={Button_Months}
              onChangeText={(cocMonths) => {
                this.changeCashInChangeData(cocMonths);
              }}
  ...
  render() {
    ...
    let total = 0;
    if (
      cashInChangeData.cicData.totalCash != undefined &&
      cashInChangeData.cicData.totalCash != null
    ) {
    ...
    if (cashInChangeData.isFetched == true) {
      for (let i = 0; i < cashInChangeData.cicData.cash.length; i++) {
        if (
          cashInChangeData.cicData.cash[i].amount != 0 &&
          cashInChangeData.cicData.cash[i].amount != -0
        ) {
