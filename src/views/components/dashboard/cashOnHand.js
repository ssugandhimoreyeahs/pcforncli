...
import { heightPercentageToDP as Height,
...
renderOutOfCashDate = React.memo(({ outOfCashDate })=>{
  return(
    <View style={{ backgroundColor: this.getRunwayColor(0,0,0),...styles.outOfCashDateDateStyle}}>
...
cashOnHandBody = React.memo(({ height,cashOnHandGraphData,past,future,currentBalance })=>{

  return(
    <View style={{ height }}>
      <View style={{ flexDirection:"row",justifyContent:"space-between",alignSelf:"center",width:"90%" }}>
         
         <TouchableOpacity style={{ flexDirection:"row" }} onPress={this.showAlert2}>
         <Text style={{ fontWeight:"500",fontSize:13,color:"#1D1E1F" }}>{`CASH ON HAND`}</Text>
         <Ionicons name='md-information-circle-outline' style={{height:12,width:12,marginLeft:3,marginTop: 2}}/>
         </TouchableOpacity>

         <View>
             
             <Text style={{ fontSize:22,color:"#1D1E1F",fontWeight:"700" }}>{`$${ numberWithCommas(currentBalance) || 0.0}`}</Text>
    ...
    <Fragment>
      <View style={styles.cashOnHandFooter}>
          ...
          <TouchableOpacity style={styles.Toucha} onPress={()=>{ this.dropdownRef.current.focus(); }}>
                ...
                <SimpleLineIcons name={arrowStyle} color="#030538" style={{marginTop:10, marginRight:20,}}/>
          </TouchableOpacity>
...

       {
          ...
          <Text style={styles.insightCartBodyText}>Your cash balance has decreased more than 50% from last month.</Text>

          <TouchableOpacity style={styles.insightCartBodyGotoButton}>
            <Text style={styles.insightCartBodyGotoButtonText}>How to improve</Text>
...
emptyCashOnHandView = React.memo(({ height })=>{
  return(
    <View style={{ height,justifyContent:"center",alignItems:"center" }}>
      <Text style={{ fontSize: 14,color:"#070640" }}>No Data Available!</Text>
    </View>
  );
});
renderCashOnHand = React.memo(({ currentBalance,parentLoader,isCOHGraphEmpty,childLoader,arrowStyle,btnText,cashOnHandGraphData, past, future })=>{
    ...
    return(
      <View style={{ ...styles.cashOnHandNewCantainer,
        ...
        <Fragment>
        {
          ...
          <Fragment>
            {
              childLoader ? <this.childLoader height={heightRatio} /> :
              isCOHGraphEmpty ? <this.emptyCashOnHandView height={heightRatio}/> :
              <this.cashOnHandBody 
                height={heightRatio}
                cashOnHandGraphData={cashOnHandGraphData}
                past={past}
            ...
            <this.cashOnHandFooter btnText={btnText} arrowStyle={arrowStyle} />
          </Fragment>
        }
        </Fragment>
       </View>
