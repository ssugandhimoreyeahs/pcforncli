...
class Integration extends Component {
      ...
      if (
        ...
        if (userConnectedToBank.result == true) {
          if (userData.bankStatus == "linked") {
            ...
            this.setState({
              isUserLinkedToBank: true,
              isUserUnlinkedToBank: false,
              bankData: userConnectedToBank.bankConnectedData,
              isSpinner: false,
              isBodyLoaded: true,
            });
          } else {
    ...
    return (
      <View style={bankNotConnectedStyle.integrationView}>
        <View style={bankNotConnectedStyle.innerViews}>
          <View style={{ flexDirection: "column", width: "64%", height: 30 }}>
            <Text style={{ fontWeight: "bold" }}>Bank Integration</Text>
          </View>
          <View style={{ flexDirection: "column", width: "36%", height: 30 }}>
            ...
            >
              <Text style={{ paddingLeft: "25%" }}>Connect</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
    ...
    return (
      <View style={bankConnectedStyle.integrationViewQB}>
        <View style={{ ...bankNotConnectedStyle.innerViews, marginTop: "1%" }}>
          <View style={{ flexDirection: "column", width: "64%", height: 30 }}>
            <Text style={{ fontWeight: "bold" }}>QuickBooks</Text>
          </View>
          <View style={{ flexDirection: "column", width: "36%", height: 30 }}>
            <TouchableOpacity
              onPress={() => {
    ...
    return (
      <View style={bankConnectedStyle.integrationViewQB}>
        <View style={bankConnectedStyle.innerViews}>
          <View style={{ flexDirection: "column", width: "58%", height: 60 }}>
            <Text style={{ fontWeight: "bold", fontSize: 16 }}>QuickBook</Text>
          </View>
      ...
      async () => {
        ...
        if (isUserSuccessfullyReLinked.result == true) {
          this.setState(
            ...
            () => {
              setTimeout(() => {
                this.showSuccessfullyPopup(
                  "Success",
                  "You have successfully connected your bank account."
                );
              }, 100);
            }
          );
          ...
          this.setState(
            ...
            () => {
              setTimeout(() => {
                this.showSuccessfullyPopup(
                  "Success",
                  "You have successfully disconnected your bank account."
                );
              }, 100);
            }
          );
      ...
      <View style={bankConnectedStyle.integrationView}>
        <View style={bankConnectedStyle.innerViews}>
          ...
          <View style={{ flexDirection: "column", width: "40%", height: 30 }}>
            <TouchableOpacity
              onPress={() => {
                this.showConfirmPopupBeforeAction(
                  "Reconnect",
                  SURE_RELINK_ACCOUNT,
                  "RELINK"
                );
  ...
  render() {
    ...
    const {
      qbIntegrationStatus,
      bankStatus: onlyForRenderBankStatus,
...
// const mapStateToProps = state => ({reduxState:state});
// const mapDispatchToProps = dispatch => ({ reduxDispath:dispatch });
// export default connect(mapStateToProps,mapDispatchToProps)(Integration,styles.container);
