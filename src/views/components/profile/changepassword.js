    ...
    if(this.state.password != this.state.confirmNewPassword){
        ...
        if(isPasswordUpdated.result == true){
          ...
          this.setState((prevState)=>{ return { isSpinner:!prevState.isSpinner,isButtonEnabled:!prevState.isButtonEnabled } },()=>{ setTimeout(()=>{
            ...
            if(isPasswordUpdated.error && isPasswordUpdated.error.code == "ECONNABORTED"){
                this.setState((prevState)=>{ return { isSpinner:!prevState.isSpinner } },()=>{ setTimeout(()=>{
                  this.showAlert("",CONNECTION_ABORTED);
                },200) });
              }else if(isPasswordUpdated.error.response != undefined && isPasswordUpdated.error.response.status == 401){
                this.setState((prevState)=>{ return { isSpinner:!prevState.isSpinner } },()=>{ setTimeout(()=>{
  ...
  render() {
    return (
      <Fragment>
        ...
        <View style={{flexDirection:'row', width:'70%',marginTop:'1%',alignSelf:"flex-start",justifyContent:'space-between'}}>
...
export default DetectPlatform(Changepassword,styles.container);