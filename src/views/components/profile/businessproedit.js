...
import { View, Text, StyleSheet, Image,TouchableOpacity, BackHandler,Keyboard, SafeAreaView,Alert, StatusBar,Platform } from "react-native";
...
AntDesign.loadFont();
 ...
 class BusinessproEdit extends React.Component{
  ...
  static navigationOptions=({navigation})=>{
    return{     
     headerTitleStyle: { marginRight:5,width:128, height:22, fontSize:17},
      headerLeft:
...
handleButtonDisabled = () => {
   const { company,industry,businessModel,companySize,yearFounded,stateIncorporated,userData } = this.state;
   //if( company in userData ){
    if(this.state.selectLogoByUser != null ){
      return false;
    }else{
      if(company == userData.company && industry == userData.industry && businessModel == userData.businessModel && companySize == userData.companySize && yearFounded == userData.yearFounded && 
 ...
 handleUpdateBtn = async () => {
  ...
  const body = {
    ...
    render(){
      const { company,industry,businessModel,companySize,yearFounded,stateIncorporated } = this.state;
      ...
      let readyProfileUrl = isBodyLoaded && (selectLogoByUser == null ? (logoByApi   == null || logoByApi == "") ? { "type":"require" }   :  { "type":"uri", uri : logoByApi } : { "type":"uri", uri: selectLogoByUser.uri } );
        ...
        <Text style={{ fontSize:17,fontWeight:"bold",color:"black" }}>{ `Business Profile` }</Text>
           ...
           <View style={styles.imgView} >
              ...
              { ( readyProfileUrl != null && readyProfileUrl.type != undefined && readyProfileUrl.type == "uri" ) ? <Image 
          ...
          <TouchableOpacity onPress={()=>{ this.handleEditImage(); }}><Text style={{color:'#007AFF',alignSelf:'center',marginVertical:'3%', fontSize:15, fontWeight:'bold'}}>Edit Logo</Text></TouchableOpacity>
          ...
          <View style={styles.detailsContainer}>
            ...
            <View style={styles.detailsRow}>
              ...
              <Dropdown
                ...
                onChangeText={(text)=>{ this.setState({ stateIncorporated: text }) }}
                onBlur={()=>{ Keyboard.dismiss(); }}
              onFocus={()=>{ Keyboard.dismiss(); }}
              dropdownPosition={-5.4}
              />
          ...
          <Button buttonStyle={styles.button}  disabledStyle={{backgroundColor:'#7FBDFF',}}  onPress={ this.handleUpdateBtn } title={"Save"} containerStyle={styles.buttonContainer} disabled={this.handleButtonDisabled()}/>
          </ScrollView>
           }
        </React.Fragment>
        );
...
export default connect(null,mapDispatchToProps)(DetectPlatform(BusinessproEdit,styles.container));
