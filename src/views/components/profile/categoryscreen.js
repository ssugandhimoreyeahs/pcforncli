...
import { View, Text,TouchableOpacity,Alert, Keyboard, StyleSheet, ScrollView} from "react-native";
...
class CategoryScreen extends React.Component{
    ...
    onTog = () =>{
        this.setState( (prevState,toggle) => { return { toggle: !prevState.toggle } } );
    }
     FirstViewComponent = () => {
        return(
            <View style={{width:"95%",height:'10%', elevation:10, backgroundColor:'#FFFFFF', alignSelf:'center', marginTop:24,justifyContent:'space-between', flexDirection:'row', alignItems:'center'}}>
              <TouchableOpacity onPress={()=>{ this.setState((prevState)=>{ return { addCategoryDialogVisible: !prevState.addCategoryDialogVisible } }) }}><Text style={{fontSize:12, color:'#000000', marginLeft:12}}>Add Category</Text></TouchableOpacity>
           </View>
        )
      }
      SecondViewComponent = () => {
        return(
            <View style={{width:"95%",height:'10%', elevation:10, backgroundColor:'#FFFFFF', alignSelf:'center', marginTop:24, }}>
                <View style={{width:'95%', alignSelf:'center', flexDirection:'row', justifyContent:'space-between' ,marginTop:13 }}>
                    <View style={{flexDirection:'column', justifyContent:'space-between',  }}>
                        <Text style={{fontSize:12, color:'#000000'}}>Apply change to all similar transactions</Text>
                        <Text style={{fontSize:10, color:'#000000'}}>Name of Recipient</Text>
                    </View>
                    <View>
                        <ToggleSwitch
    ...
    handleCategoryDataEdit = async (categoryId,categoryName) => {
        this.setState({ editInitDialogId: categoryId,editInitDialogValue:categoryName },()=>{
    ...
    triggerAddCategoryToTransaction = async (categoryId,categoryName) => {
        ...
        const changePlaidCategoryResponse = await addCategoryToTransaction(_id,categoryId);
    ...
    CategoryComponent = (props) => {
        ...
        const { category } = this.props.navigation.getParam("currentExecutingTransaction");
        let readyCheck = handleCategoryChangeValue.length == 0 ? category : handleCategoryChangeValue;
        return(
            <React.Fragment>
            {
                ...
                <View style={{justifyContent:'space-between', flexDirection:'row', width:'90%', alignSelf:'center'}}>
                   ...
                   {
                       this.state.isUsefulTouched == true && props.showEditTray == true ? 
                       <Fragment>
                                <View style={{flexDirection:'row'}}>
                                <TouchableOpacity onPress={()=>{ this.handleCategoryDataEdit(props.categoryId,props.categoryName); }} >
                                    <View style={{backgroundColor:'#FFFFFF', width:34,height:30, }}><EvilIcons name='pencil' size={23} color={'#000000'}/></View>
                                </TouchableOpacity>
                                <TouchableOpacity onPress={()=>{ this.handleCategoryDataDelete(props.categoryId,props.categoryName); }}>
                                    <View style={{backgroundColor:'#FFFFFF', width:34,height:30, }}><MaterialIcons name='delete' size={20} color={'#000000'}/></View>
                                </TouchableOpacity>
                                </View>
                       </Fragment> : null
                   }
            ...
            <TouchableOpacity style={{width:'100%', alignSelf:'center',...readyDesign }} onPress={()=>{ this.handleAddCategoryToTransaction(props.categoryId,props.categoryName); }}>
                <View style={{justifyContent:'space-between', flexDirection:'row', width:'90%', alignSelf:'center'}}>
                    <Text style={{marginLeft:12}}>{props.categoryName}</Text> 
                   {
                       ...
                       <Fragment>
                                <View style={{flexDirection:'row'}}>
                                <TouchableOpacity >
                                    <View style={{backgroundColor:'#FFFFFF', width:34,height:30, }}><MaterialIcons name='check' size={20} color={'#000000'}/></View>
                                </TouchableOpacity>
                                </View>
                       </Fragment> : null
                   }
    ...
    render(){
        ...
        const { addCategoryDialogVisible,isSpinner,editCategoryDialogVisible,editInitDialogValue,showPleaseEnterCategory } = this.state;
        ...
        return(
            <View style={Styles.margins}>
                ...
                <DialogInput isDialogVisible={editCategoryDialogVisible}
                    ...
                    submitInput={ (editInitDialogNewValue) => {
                     ...
                     this.setState({ editCategoryDialogVisible:false,isSpinner: true },()=>{
                ...
                <ScrollView>
                    {
                                categoryReduxData.loading == true  ?  <ActivityIndicator color="#070640" size={'large'} style={{marginTop:30}} />  :
                               <View style={{ width:"95%",elevation:10, backgroundColor:'#FFFFFF', alignSelf:'center', marginTop:24,justifyContent:'space-between', flexDirection:'column', marginVertical:24 }}>
                                {
                                    categoryReduxData.isFetched == true && categoryReduxData.error == false ?
                                    <Fragment>
                                {
                                categoryReduxData.category.map( (singleCategoryData,indexing) => {
                                    return <this.CategoryComponent key={indexing} index={ indexing == 0 ? true : false} categoryName={ `${singleCategoryData.categoryName}` } categoryId={ singleCategoryData.id }  showEditTray={ singleCategoryData.customcategories == true ? true : false } />
                                })
                                }
                                </Fragment>
                                    :null
...
const Styles = StyleSheet.create({
    ...
    export default connect(mapStateToProps,mapDispatchToProps)(DetectPlatform(CategoryScreen,Styles.margins));