...
import { View, Text, Button, StyleSheet, Image,TouchableOpacity, BackHandler, SafeAreaView, StatusBar } from "react-native";
import { Button as NativeButton, Card, Input, Text as Txt, Icon } from "react-native-elements";
...
class Businesspro extends React.Component{
  constructor(props){
    ...
    render(){
      const { userData,loading,isFetched,isError } = this.props.reduxState.userData;
      const { isDisabled } = this.state;
      const readyImageAssets = ( userData.logo == "" || userData.logo == undefined || userData.logo == null ) ? { type:"require" } : { type:"uri" };
      
      return(
          <React.Fragment>
             <Spinner
                visible={loading && !isFetched }
             />
        ...
        <View style={{ paddingHorizontal:10,alignSelf:"center",marginTop: 15,flexDirection:"row",width:"100%",justifyContent:"space-between",borderWidth:0,borderColor:"black" }}>
                        ...
                        <View style={{ justifyContent:"center",alignItems:"center" }}>
                            <TouchableOpacity onPress={()=>{ 
                              this.props.navigation.navigate("BusinessproEdit",{ userData:userData }) 
                             }}>
                              <Text style={styles.editView}>Edit</Text>
                            </TouchableOpacity>
                        </View>
           ...
           <View style={styles.imgView}>
              ...
              { (  readyImageAssets.type == "uri" ) ? <Image 
          ...
          <View style={styles.detailsContainer}>
            <View style={styles.detailsRow}>
              ...
              <Dropdown
                label={ this.renderValues(userData.industry).length == 0 ?  "Select                   v" : "" }
                disabled={isDisabled}
            ...
            <View style={styles.detailsRow}>
              ...
              <Dropdown
                disabled={isDisabled}
                label={  this.renderValues(userData.businessModel).length == 0 ? "Select                   v" : "" }
            ...
            <View style={styles.detailsRow}>
              ...
              <Dropdown
                disabled={isDisabled}
                label={ this.renderValues(userData.companySize).length == 0  ? "Select                   v" : "" }
            ...
            <View style={styles.detailsRow}>
              ...
              <Dropdown
                disabled={isDisabled}
                label={ this.renderValues(userData.yearFounded).length == 0 ? "Select                   v" : "" }
            ...
            <View style={styles.detailsRow}>
              ...
              <Dropdown
                disabled={isDisabled}
                label={ this.renderValues(userData.stateIncorporated).length == 0 ? "Select            v" : "" }
...
export default connect(mapStateToProps,mapDispatchToProps)(DetectPlatform(Businesspro,styles.container));
