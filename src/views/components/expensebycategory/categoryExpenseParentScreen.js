...
import { Button_Months,EXPENSES_COLOR_CODE,Button_MonthsSUB } from "../../../constants/constants";
...
class CategoryExpenseParentScreen extends Component{
    ...
    static getDerivedStateFromProps(props, state){
        ...
        //code here
        const { mainExpenseType: expenseCurrentRange } = props.mainExpenseByCategoryRedux;
    ...
    componentDidMount = () => {
        ...
        //dashboard expense
        const { expense,expenseCurrentRange: dashboardExpenseCurrentRange,totalExpense } = this.props.expenseByCategoryRedux;


        const { mainExpenseType: expenseCurrentRange } = this.props.mainExpenseByCategoryRedux;
      ...
      handleBackButton=(nav)=> {
        if(!nav.isFocused()) {
          ...
          if(this.state.showSubExpenseCategoryState.isLogicStart == true){
            this.setState({ showSubExpenseCategoryState:{ ...this.state.showSubExpenseCategoryState,isLogicStart: false } });
          }else{
            nav.goBack();
          }
    ...
    handleChangeSubMainExpense =  (mainSubExpenseText) => {
        const { category }   = this.state.showSubExpenseCategoryState.parentExpenseCategoryData;
        const { currentExecutingSubExpenseMonth } = this.state.showSubExpenseCategoryState;
        ...
        if( currentExecutingSubExpenseMonth != mainSubExpenseText ){
            ...
            const triggerSubExpenseCategoryApiResponse = await getSubCategories(readyParam,category);
                ...
                this.setState((prevState)=>{
                    return {
                        ...
                        showSubExpenseCategoryState:{
                            ...prevState.showSubExpenseCategoryState,
                            showSubExpenseCategoryLoader:false,
                            subExpenseCategoryData:triggerSubExpenseCategoryApiResponse.subCategoryResponse
                        }
                    }
                })
            }else{
    ...
    handleReloadMainExpenseByCategory = () => {
        const { currentExecutingMainExpenseMonth:currentExpenseMainRange } = this.state;
    ...
    triggerChildCategoryLogic = async (allSingleParentCategoryData) => {
        ...
        //console.log("final test here ----------------------------------------------------",this.state.currentExecutingMainExpenseMonth);
        const triggerSubExpenseCategoryApiResponse = await getSubCategories(expenseCurrentRange,allSingleParentCategoryData.category);
        if(triggerSubExpenseCategoryApiResponse.result == true){
            this.setState((prevState)=>{
                return {
                    sampleSpinner:false,
    ...
    renderMainExpenseSingleData = ({ category,amount,colorCode,allSingleParentCategory }) => {
        ...
        return(
            <Fragment>
                <View style={{ flexDirection:"row",justifyContent:"space-between",alignItems:"center" }}>
                            ...
                            }} style={{ justifyContent:"center",alignItems:"center",width:175,height:35,borderRadius:50,borderColor:"#F98361",backgroundColor:`${colorCode}` }}>
                                        <Text style={{ fontSize:15,lineHeight:22,height:22,color:"#FFFFFF" }}>{ category }</Text>
                            </TouchableOpacity>
                            <Text style={{ fontSize:15,lineHeight:22,height:22,direction:"rtl" }}>{ `-$${numberWithCommas(amount)}` }</Text>
                    </View>
                <Seprator />
            </Fragment>
        );
    ...
    renderMainExpenseSubSingleData = ({ category,amount,colorCode,allSingleSubCategory }) => {
        ...
        return(
            <Fragment>
                <View style={{ flexDirection:"row",justifyContent:"space-between",alignItems:"center" }}>
                            <TouchableOpacity onPress={()=>{
                                this.props.navigation.navigate("ExpenseScreenChild",{ parentExpenseCategory:this.state.showSubExpenseCategoryState.parentExpenseCategoryData,subExpenseCategory:allSingleSubCategory,currentExecutingSubCategory:this.state.showSubExpenseCategoryState.currentExecutingSubExpenseMonth })
                            }} style={{ 
                                //justifyContent:"center",alignItems:"center",width:200,height:35,borderRadius:50,borderColor:"#F98361",backgroundColor:`${colorCode}` 
                                paddingHorizontal:20,paddingVertical:10,width:200,justifyContent:"center",alignItems:"center",borderRadius:50,borderColor:"#F98361",backgroundColor:`${colorCode}`
                                }}>
                                        {/* <Text style={{ fontSize:15,lineHeight:22,height:22,color:"#FFFFFF" }}>{ category }</Text> */}
                                         <Text style={{ fontSize:15,color:"#FFFFFF",textAlign:"center" }}>{ category }</Text>   
    ...
    renderAllExpensesList = () => {
        let { error,isFetched,loading,mainExpenses } = this.props.mainExpenseByCategoryRedux;
        ...
        let { isLogicStart,subExpenseCategoryData } = this.state.showSubExpenseCategoryState;
        let readySubExpenseCategoryData;
        if(isLogicStart == true){
            readySubExpenseCategoryData = subExpenseCategoryData.ExpenseByCategory.map((singleSubCategoryExpense,index) => {  return { ...singleSubCategoryExpense,colorCode:colorCode[index] }   });
        }
        return(
            <Fragment>
                {
                    ...
                    <View style={{ marginVertical:30,width:"90%",alignSelf:"center" }}>
                    {
                        ...
                        mainExpensesChange.map( (singleMainExpense,index) => {
                            return <this.renderMainExpenseSingleData key={index} category={allFirstWordCapital(singleMainExpense.category)} amount={singleMainExpense.amount} colorCode={singleMainExpense.colorCode} allSingleParentCategory={singleMainExpense} />
                        }) 
                    }
                    </View> : null
                }

                {
                    ...
                    <View style={{ marginVertical:30,width:"90%",alignSelf:"center" }}>
                    {
                        ...
                        readySubExpenseCategoryData.map( (singleSubMainExpense,index) => {
                            return <this.renderMainExpenseSubSingleData key={index} category={allFirstWordCapital(singleSubMainExpense.category)} amount={singleSubMainExpense.amount} colorCode={singleSubMainExpense.colorCode} allSingleSubCategory={singleSubMainExpense} />
                        })
                    }
                    </View> : null
                }
    ...
    handleArrowStyle = () => {
     ...
     handleSubCategoryLogicReset = ()=> {  
       ...
       // console.log("logic false Ends Here")
        this.setState({ currentExecutingMainExpenseMonth:renderButton,showSubExpenseCategoryState:{ ...this.state.showSubExpenseCategoryState,isLogicStart: false,showSubExpenseCategoryLoader:false,
            currentExecutingSubExpenseMonth:"This Month" } })
    ...
    render(){
        ...
        let { showSubExpenseCategoryLoader,isLogicStart,parentExpenseCategoryData,currentExecutingSubExpenseMonth,subExpenseCategoryData } = this.state.showSubExpenseCategoryState;
        // console.log(this.props.mainExpenseByCategoryRedux);
        const { error,isFetched,loading,mainExpenses,totalMainExpense,mainExpenseType } = this.props.mainExpenseByCategoryRedux;
        //label:`${singleMainExpense.category}\n${parseInt(singleMainExpense.percentage)}%`,
        //label:`${singleMainExpense.category}\n${parseInt(singleMainExpense.percentage)}%`,
        const generatePieChart = mainExpenses.map((singleMainExpense,index) => { return { x:index,y:parseInt(singleMainExpense.percentage) } });
        let generateSubPieChart;
        if(isLogicStart == true){
            generateSubPieChart = subExpenseCategoryData.ExpenseByCategory.map( (singleSubMainExpense,index)=>{ return { x:index,y:parseInt(singleSubMainExpense.amount) } });
        }
        return(
            <ScrollView style={styles.container} >
                ...
                <View style={{ marginTop: 20,flexDirection:"row",width:"100%" }}>
                    ...
                    <View style={{ marginTop: 20,flexDirection:"row",width:"100%" }}>
                        ...
                        <View style={{ width:"80%",justifyContent:"center",alignItems:"center" }}>
                            <Text style={{ fontSize:17,lineHeight:22 }}>{ `${allFirstWordCapital(parentExpenseCategoryData.category)} Expenses` } </Text>
                        </View>
                    </View>
                ...
                {
                    error == true ?  <View style={{ justifyContent:"center",alignItems:"center",marginTop:300 }}>
                    ...
                    loading == true ? <View style={{ justifyContent:"center",alignItems:"center",marginTop:300 }}><ActivityIndicator size="large" animating={loading} color="#070640" /></View> :
                    isFetched == true ? <Fragment>
                        <View style={{ backgroundColor:"#FFF",height:400,alignItems:"center",width:"100%", }}>
...

                            {
                                ...
                                <VictoryPie
                                    ...
                                    style={{ data:{ width: 3 },labels: { fill: "black", fontSize: 15, fontWeight: "200",paddingLeft:20 } }}
                                    events={
                                        [{
                                            target: "labels",
                                            eventHandlers: {
                                                        ...
                                                        return [
                                                            {
                                                                ...
                                                                mutation: ({ style }) => {
                                                                    ...
                                                                    if(style.fontWeight == "200"){
                                                                        return { style:{ ...style, fontWeight:"600"  } }
                                                                    }else{
                                                                        return { style:{ ...style, fontWeight:"200"  } }
                                                                    }
                                    ...
                                    <VictoryLabel
                                        textAnchor="middle"
                                        style={{ fontSize: 22,height:20,lineHeight:20,fontWeight:"600" }}
                                        x={(gw-20)/2} y={150}
                                        text={ isLogicStart == false ? `-$${(numberWithCommas(parseInt(totalMainExpense)))}` : `-$${(numberWithCommas(parseInt(subExpenseCategoryData.amount)))}`}
                                        />
                                </Svg> 
                                
                                : isLogicStart == false && generatePieChart.length == 0 ? <View style={{ height:250,justifyContent:"center",alignItems: "center", }}><Text>No Data Available!</Text></View> : null
                                ...
                                : isLogicStart == true && generateSubPieChart.length == 0 ? <View style={{ height:250,justifyContent:"center",alignItems: "center", }}><Text>No Data Available!</Text></View> : null
...

                    {
                        ...
                        <Dropdown
                                ...
                                data={Button_Months}
                                onChangeText={(mainExpenseText)=>{ this.handleChangeMainExpense(mainExpenseText); }}
                                value={ isLogicStart == false && this.state.currentExecutingMainExpenseMonth}
                        ...
                        <Dropdown
                                ...
                                onChangeText={(mainSubExpenseText)=>{ this.handleChangeSubMainExpense(mainSubExpenseText); }}
                                value={ isLogicStart == true && currentExecutingSubExpenseMonth }
                                containerStyle={styles.dropdown}
                                renderAccessory={() => null}
                                pickerStyle={{backgroundColor:"#E6E6EC",borderRadius:10,}}
                                onBlur={()=>{  Keyboard.dismiss(); }}
                ...
                <View style={{ marginTop:24,marginLeft:20,marginRight:20,backgroundColor:"#FFF",flexDirection:"column" }}>
                    {
                        this.state.sampleSpinner == true ?
                        <View style={{ marginVertical:30,width:"90%",alignSelf:"center" }}><ActivityIndicator animating={this.state.sampleSpinner}/></View>
...
export default connect(mapStateToProps,mapDispatchToProps)(DetectPlatform(CategoryExpenseParentScreen));