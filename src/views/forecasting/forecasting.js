...
import { StyleSheet, View, BackHandler,Alert,TouchableOpacity } from "react-native";
...
class Forecasting extends Component {
  ...
  loadForecastData = () => { 
    getForecastData().then((forecastRes)=>{
      if(forecastRes.result == true){
        let { forecastResponse } = forecastRes;
        console.log("getting forecast response here ------ ",forecastResponse);
        ...
        this.setState({ isPositiveCashComeFromApi,forecastResponse,cash: forecastResponse.cashOnHand,expenses: Math.abs(forecastResponse.expense),sales: Math.abs(forecastResponse.sales),oocLabel: forecastResponse.outOfCashDate,isCommingPositiveCashFlow },()=>{
          setTimeout(()=>{
            this.setState({ isSpinner: false, isError: false,isBodyLoaded: true });
          },100);
        });  
  ...
  getUpdatedForecastLocal(oocLabel,cash, expenses, sales, isPositive){
      ...
      let responseData = getUpdatedForecast(oocLabel,cash, expenses, sales, this.state.forecastResponse.cashOnHand,this.state.forecastResponse.outOfCashDate, this.state.isCommingPositiveCashFlow);
    ...
    if(responseData.updatedOutofCashDate == "Positive Cash Flow"){
      ...
      this.setState({ isPositiveCashFlow: false,isCommingPositiveCashFlow : false });
  ...
  getRunwayColor(cash, expenses, sales) {
    ...
    if(this.state.isPositiveCashFlow == true || this.state.isCommingPositiveCashFlow == true){
     // console.log("getting the postive cash flow from the api end point");
      return `#47C4D3`;
    }else if(this.state.oocLabel == this.state.forecastResponse.outOfCashDate){
      return `#47C4D3`;
    }else{
      if(new Date(this.state.forecastResponse.outOfCashDate).getTime() < new Date(this.state.oocLabel).getTime()){
        return `#47C4D3`;
      }else{
        return `#FF5E3A`;
      }
  ...
  handleResetForecast = () => {
      ...
      this.setState((prevState)=>{
        ...
        // }
        this.cohRef.current.methodToBeRunAfterReset(forecastResponse.cashOnHand);
        this.expenseRef.current.methodToBeRunAfterReset(Math.abs(forecastResponse.expense));
        this.salesRef.current.methodToBeRunAfterReset(Math.abs(forecastResponse.sales));
      })
  ...
  render() {
    ...
    return (
      ...
      <View style={styles.container}>
            <View style={{ justifyContent:"center",alignItems:"center", marginTop:"60%",height:300 }}>
            <View style={{ flexDirection:"row",justifyContent:"center",alignItems:"center" }} >
                <AntDesign name="exclamationcircle" size={20} style={{ color:'#070640',alignSelf:"center" }}/>
                <Text style={{ marginLeft:10,alignSelf:"center" }}>You are not connected to Bank!</Text>
            </View> 
        ...
        <View style={styles.container}>
          ...
          >
            ...
            <Text style={styles.headerValue}>{ 
              ...
              this.state.isCommingPositiveCashFlow == true ?  `Positive Cash Flow`  : this.state.isPositiveCashFlow == true ? `Positive Cash Flow` : getFormatedDate(this.state.oocLabel)
            }</Text>
          </View>
          <View 
              style={{
          ...
          : isError == true ? <View style={styles.container}>
            <View style={{ justifyContent:"center",alignItems:"center", marginTop:"60%",height:300 }}>
            <View style={{ flexDirection:"row",justifyContent:"center",alignItems:"center" }} >
                <AntDesign name="exclamationcircle" size={20} style={{ color:'#070640',alignSelf:"center" }}/>
