...
export async function getUserTransactions(SKIP = null, LIMIT = null) {
  try {
    ...
    if (data.success == true) {
      return { result: true, transactions: data.transactions, totalTransactions: data.totalTransactions, accountType: data.accountType };
    } else {
...
export async function fetchCurrentBalance() {
  try {
    ...
    if (response.data.succses == true) {
      return { result: true, available_balance: response.data["available balance"] };
    } else {
      return { result: false, response: response.data };
    }
...
export function fetchCurrentBalancePromise() {
  ...
  return new Promise((resolve, reject) => {
    AsyncStorage.getItem("authToken").then((token) => {
      ...
      axios.get(APINETWORK.getCurrentBal, {
        headers: { Authorization: token, "Content-Type": "application/json" },
        timeout
      }).then((response) => {
        // setTimeout(()=>{
        ...
        if (response.data.succses == true && response.status == 200) {
          resolve({ result: true, available_balance: response.data["available balance"] });
        }
        else {
          reject({ result: false });
        }
...
export async function unlinkBankAccount() {
  try {
    const token = await AsyncStorage.getItem("authToken");
    const response = await axios.post(APINETWORK.unlinkBank, {}, {
      headers: { Authorization: token, "Content-Type": "application/json" },
      timeout
    });
    const res = response.data;
...
export async function reLinkBankAccount() {
  try {
    const token = await AsyncStorage.getItem("authToken");
    const response = await axios.post(APINETWORK.relinkBank, {}, {
      headers: { Authorization: token, "Content-Type": "application/json" },
      timeout
    });
    const res = response.data;
...
export function validatePlaidTokenPromise() {
  ...
  return new Promise((resolve, reject) => {
    AsyncStorage.getItem("authToken").then((token) => {
      ...
      axios.get(APINETWORK.plaidTokenStatus, {
        headers: { Authorization: token, "Content-Type": "application/json" },
        timeout
        ...
        console.log("Valid Plaid Token Api response - ", validatePlaidToken.data);
        if (validatePlaidToken.data.success == true && validatePlaidToken.status == 200) {
...
export async function getCashOutOfDate() {
  try {
    const token = await AsyncStorage.getItem("authToken");
    const triggerOutOfCashApiResponse = await axios.get(APINETWORK.outOfCashDateApi, {
      headers: { Authorization: token, "Content-Type": "application/json" },
      timeout
    });
    ...
    if (triggerOutOfCashApiResponse.data.success == true) {
      return { result: true, outOfCashDate: triggerOutOfCashApiResponse.data.days };
    } else {
      return { result: false };
    }
  } catch (error) {
...
export async function getUserOutFlowTransactions(skip = 0, limit = 0) {
  try {
    ...
    const token = await AsyncStorage.getItem("authToken");
    let generateOutflowUrlWithSkip = `${APINETWORK.getOutflowTransactions}&skip=${skip}&limit=${limit}`;
    const outflowTransactionsResponse = await axios.get(generateOutflowUrlWithSkip, {
      headers: { Authorization: token, "Content-Type": "application/json" },
      timeout
    });
    if (outflowTransactionsResponse.status == 200 && outflowTransactionsResponse.data.success == true) {
      console.log(`Outflow Response url = ${generateOutflowUrlWithSkip} - response length - ${outflowTransactionsResponse.data.transactions.length}`);
      return { result: true, outflowTransactions: outflowTransactionsResponse.data };
    } else {
      return { result: false };
    }
  } catch (error) {
...
export async function getUserInflowTransactions(skip = 0, limit = 0) {
  try {
    ...
    const token = await AsyncStorage.getItem("authToken");
    let generateInflowUrlWithSkip = `${APINETWORK.getInflowTransactions}&skip=${skip}&limit=${limit}`;
    const inflowTransactionsResponse = await axios.get(generateInflowUrlWithSkip, {
      headers: { Authorization: token, "Content-Type": "application/json" },
      timeout
    });
    if (inflowTransactionsResponse.status == 200 && inflowTransactionsResponse.data.success == true) {
      console.log(`Inflow Response url = ${generateInflowUrlWithSkip} - response length - ${inflowTransactionsResponse.data.transactions.length}`);
      return { result: true, inflowTransactions: inflowTransactionsResponse.data };
    } else {
      return { result: false };
    }
  } catch (error) {
...
export async function getPlaidCategory() {
  try {
    ...
    console.log("-----------------------------------------------------------------------");
    console.log("Plaid Category Fetched Status - ", getPlaidCategoryResponse.status);
    console.log("Plaid Category Data - ", getPlaidCategoryResponse.data);
    console.log("-----------------------------------------------------------------------");

    if (getPlaidCategoryResponse.status == 200 && getPlaidCategoryResponse.data.success == true) {


      return { result: true, plaidCategoryData: getPlaidCategoryResponse.data.category };
    } else {
      return { result: false };
    }
  } catch (error) {
...
export async function addPlaidCategory(category) {
  try {
    ...
    const token = await AsyncStorage.getItem("authToken");
    const getPlaidCategoryResponse = await axios.post(APINETWORK.addCategory, { category }, {
      headers: { Authorization: token, "Content-Type": "application/json" },
      timeout
    });
...
export async function deletePlaidCategory(id) {
  try {
    ...
    if (deletePlaidCategoryResponse.status == 200 && deletePlaidCategoryResponse.data.success == true) {
...
export async function editPlaidCategory(id, category) {
  try {
    ...
    const token = await AsyncStorage.getItem("authToken");
    const editPlaidCategoryResponse = await axios.post(APINETWORK.editCategory, { id, category }, {
      headers: { Authorization: token, "Content-Type": "application/json" },
      timeout
    });
    if (editPlaidCategoryResponse.status == 200 && editPlaidCategoryResponse.data.success == true) {
...
export function getPlaidCategoryUsingPromise() {
  ...
  return new Promise((resolve, reject) => {
    ...
    AsyncStorage.getItem("authToken").then(token => {
      ...
      axios.get(APINETWORK.getCategory, {
        headers: { Authorization: token, "Content-Type": "application/json" },
        timeout
        ...
        if (getPlaidCategoryResponse.status == 200 && getPlaidCategoryResponse.data.success == true) {
          ...
          resolve({ result: true, plaidCategoryData: getPlaidCategoryResponse.data.category });
        } else {
          reject({ result: false });
        }
...
export async function addCategoryToTransaction(transactionId, categoryId) {
  try {
    ...
    console.log(body);
    console.log("------------------------------------");
    const addCategoryToTransactionResponse = await axios.post(APINETWORK.addCategoryToTransaction, body, {
      headers: { Authorization: token, "Content-Type": "application/json" },
      timeout
    });
    console.log("Edit category response - ", addCategoryToTransactionResponse.data);
    console.log("Edit category status - ", addCategoryToTransactionResponse.status);
    if (addCategoryToTransactionResponse.status == 200 && addCategoryToTransactionResponse.data.success == true) {
...
export async function getSubCategories(fetchType, category) {
  try {
    ...
    if (fetchType == 1) {
      readyQueryParams = `${APINETWORK.getSubCategories}?currentMonth=true&category=${category}`;
    } else if (fetchType == 3) {
      readyQueryParams = `${APINETWORK.getSubCategories}?pastThreeMonth=true&category=${category}`;
    } else if (fetchType == 6) {
      readyQueryParams = `${APINETWORK.getSubCategories}?pastSixMonth=true&category=${category}`;
    } else {
      readyQueryParams = `${APINETWORK.getSubCategories}?pastTwelveMonth=true&category=${category}`;
    }
    ...
    if (getSubCategoryResponse.status == 200 && getSubCategoryResponse.data.success == true) {
      return { result: true, subCategoryResponse: getSubCategoryResponse.data };
    } else {
      return { result: false, subCategoryResponse: getSubCategoryResponse.data };
    }
  } catch (error) {
    return { result: "error", error };
  }
...
export async function getSubCategoryTransactions(transactionType, category, subCategory) {
  try {

    const token = await AsyncStorage.getItem("authToken");
    let readySubExpenseQueryParams;
    if (transactionType == 1) {
      readySubExpenseQueryParams = `${APINETWORK.getSubCategoryTransactions}?currentMonth=true&category=${category}&subCategory=${subCategory}`;
    } else if (transactionType == 3) {
      readySubExpenseQueryParams = `${APINETWORK.getSubCategoryTransactions}?pastThreeMonth=true&category=${category}&subCategory=${subCategory}`;
    } else if (transactionType == 6) {
      readySubExpenseQueryParams = `${APINETWORK.getSubCategoryTransactions}?pastSixMonth=true&category=${category}&subCategory=${subCategory}`;
    } else {
      readySubExpenseQueryParams = `${APINETWORK.getSubCategoryTransactions}?pastTwelveMonth=true&category=${category}&subCategory=${subCategory}`;
    }
    console.log("request for the sub categories transactions  - ", readySubExpenseQueryParams);
    const getSubCategoryTransactionResponse = await axios.get(readySubExpenseQueryParams, {
      headers: { Authorization: token, "Content-Type": "application/json" },
      timeout
    });
    // console.log("Edit category response - ",getSubCategoryResponse.data);
    // console.log("Edit category status - ",getSubCategoryResponse.status);
    if (getSubCategoryTransactionResponse.status == 200 && getSubCategoryTransactionResponse.data.success == true) {
      return { result: true, subCategoryTransactionResponse: getSubCategoryTransactionResponse.data };
    } else {
      return { result: false, subCategoryTransactionResponse: getSubCategoryTransactionResponse.data };
    }
  } catch (error) {
    return { result: "error", error };
  }
