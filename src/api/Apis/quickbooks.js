...
export async function getSalesData() {
  try {
    ...
    if (getSalesResponse.data.success == true) {
      return {
        result: true,
        salesChartData: getSalesResponse.data.datamonth,
        response: getSalesResponse.data,
      };
    } else {
      ...
      .then((token) => {
        ...
        axios
          ...
          .then((getSalesResponse) => {
            if (
              getSalesResponse.data.success == true &&
              getSalesResponse.status == 200
            ) {
              resolve({
                result: true,
                salesData: getSalesResponse.data,
                salesChartData: getSalesResponse.data.datamonth,
                response: getSalesResponse.data,
              });
            } else {
...
export async function triggerNoQbForm(ledgerAccountType) {
  try {
    const token = await AsyncStorage.getItem("authToken");
    const getSalesResponse = await axios.post(
      APINETWORK.noQbForm,
      { ledgerAccountType },
      {
        headers: { Authorization: token, "Content-Type": "application/json" },
