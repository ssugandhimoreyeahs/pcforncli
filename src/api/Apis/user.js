...
//   }
  ...
  export async function isUserAlreadyExist(username){
    try{
        console.log("Gettint the isUserAlreadyExistURl Here - ",APINETWORK.isUserExist);
        const response = await axios.post(APINETWORK.isUserExist,{ username },{
          headers: { "Content-Type":"application/json " },
          timeout
        });
        const res = response.data;
  ...
  export async function forgetPassword(username){
    try{
      const response = await axios.post(APINETWORK.forgetPassword,{ username },{
        headers: { "Content-Type": "application/json" },
        timeout
      });
      const res = response.data;
  ...
  export async function userLoginCounter(){
    try{
      ...
      const token = await AsyncStorage.getItem("authToken");
      const userLoginCounterResponse  = await axios.post(APINETWORK.userLoginCounter,{ "checklogin":"true" },{
        headers: { Authorization: token,"Content-Type": "application/json" },
        timeout
      });
      ...
      if(userLoginCounterResponse.status == 200 && userLoginCounterResponse.data.success == true){
        return { result: true,userLoginCounterResponse: userLoginCounterResponse.data };
      }else{
        return { result: false };
      }
    }catch(error){
  ...
  export function getUserPromise(){
    ...
    return new Promise((resolve,reject)=>{
      AsyncStorage.getItem("authToken").then((token)=>{
        ...
        axios.get(APINETWORK.getUser, {
          headers: { Authorization: token, "Content-Type": "application/json" },
          timeout
          }).then((response)=>{
  
            if(response.data.success == true && response.status == 200){
               resolve({ result:true,userData:response.data.userData,token });
            }else{
              reject({ result:false });
            }
  ...
  export function sendUserFeedbackData(feedbackBody){
      ...
      return new Promise((resolve,reject)=>{
        ...
        AsyncStorage.getItem("authToken").then((token)=>{ 
          ...
          axios.post(APINETWORK.storeUserFeedback,feedbackBody,{
            headers: { Authorization: token },
            timeout
            }).then((response)=>{
              ...
              if(response.status == 200 && response.data.success == true){
                ...
                console.log("Error Log - sendUserFeedbackData() - response issue");
