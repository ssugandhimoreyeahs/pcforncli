...
export default class SliderField extends Component {
  ...
  methodToBeRunAfterReset = (value) => {
    this.setState({ value });
    console.log(
      "This is the sliderField method to be run after the resetting the forecast logic - "
    );
  };
  render() {
    return (
        ...
        {this.props.label == `Cash on Hand` ? (
          this.props.isShowReset == true ? (
            <TouchableOpacity
              ...
              style={{ alignSelf: "flex-end", margin: 22 }}
            >
              <Text
                style={{ color: `#0B61C8`, fontSize: 15, fontWeight: "bold" }}
              >{`Reset`}</Text>
            </TouchableOpacity>
          ) : null
        ) : null}
